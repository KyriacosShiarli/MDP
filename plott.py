...
def trajectoryCompare(examples,steps,model,name):
	...
	x = range(70)
	policy= caus_ent_backward(model.transition,model.reward_f,5,70,0.1,None) 
	for example in examples:
		traj[0,:] = example["states"][0]
		for i in range(1,example["steps"]):
			prev_state = m.quantityToState(traj[i-1,:])
			action = np.random.choice(50,p=policy[:,prev_state])
			...
			axarr[0].scatter(i,m.stateToQuantity(example["state_numbers"][i])[1],color = "red",alpha = 0.1)
			axarr[1].scatter(i,m.stateToQuantity(example["state_numbers"][i])[0],color = "red",alpha = 0.1)
		axarr[0].scatter(x,traj[:,1],color = "green",label = "learned",alpha = 0.1)
		axarr[1].scatter(x,traj[:,0],color = "green",alpha = 0.1)		
