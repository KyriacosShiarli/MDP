...
def staticGroupSimple(state,action,duration = 0.1):
	...
	x_vel_fin = action[0]*math.cos(action[1])
	y_vel_fin = action[0]*math.sin(action[1])
	...
	if action[0]==0 and state[1]==0:
		orient =state[0] - action[1]*duration
	else:
