...
policy_ref=caus_ent_backward(model.transition,model.reward_f,examples[1]["end_state"],steps)
...
for itera in xrange(iterations):
	policy_test=caus_ent_backward(model.transition,model.reward_f,examples[1]["end_state"],steps)
	...
	#X = X[np.nonzero(Y)[0]]
	#Y = Y[np.nonzero(Y)[0]]
	#ht.predict(model.feature_f.reshape(states*actions,features))
	print np.nonzero(Y)
	print Y.shape
	print "starting adaboost"
	ht = adaboost_reg(X,Y,100,4)
	print "finished adaboost"
	print "reducing gradient"
	model.reward_f = (model.reward_f.reshape(states*actions)*np.exp(-10.*Y)).reshape(actions,states)	
